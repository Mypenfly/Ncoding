...
| key/value 匹配 | 严格匹配，value 可以是任意长字符串（含换行、代码等） |
| 截止符 `<<<[__END__]>>>` | 标识命令块末尾，非严格必需——遇到下一个 `<<<[Command]>>>` 或模型输出结束也视为终止 |
...
每次发起 API 请求时，直接使用 session 文件中的 `messages` + 当前系统提示词作为上下文。若消息超长，采用**从旧到新保留最近 N 条完整消息**的截断策略（N 由配置 `max_context_messages` 决定，默认 50），system prompt 始终保留。
...
coding agent 最重要的命令。使用 **nushell** 执行，非 bash。
...
```
你的 shell 环境是 nushell (nu)，不是 bash。管道、重定向等语法请使用 nushell 语法。
...
1. 通过 `std::process::Command` 调用 `nu -c "{command}"` 执行
2. 当前工作目录为项目根目录
3. 捕获 stdout + stderr
4. 默认超时 120 秒（可配置）
5. 多个 `---` 分割的命令**并发执行**
...
command: cargo build
is_async: true
async exec the command, will notify when done
)>)>
```
...
mode: edit
path: src/main.rs
result: failed - old_str matched 2 locations (lines 32, 156). Provide more context to make old_str unique.
)>)>
```
...
└── src/
    ...
    │   ├── syntax.rs            # 数据结构 enum
    │   ├── shell.rs             # Shell 命令（nushell 执行 + 安全审查 + is_async）
...
| 1.6 | 实现 Shell 命令执行器：nu -c 执行，安全审查，同步模式（is_async=false），超时处理 | `command/shell.rs` |
...
| 4.5 | Shell 环境信息注入到 system prompt（告知 nushell、rg、jj 可用） | prompt 生成 |
...
1. **Shell 使用 nushell 而非 bash**：nushell 类型安全、结构化输出，更现代的 shell 体验
...
| Shell | bash (外部工具) | nushell（内置 + async 支持） |
| 文件操作 | ToolCall 外部工具 | FilesOperator（内置 read/write/edit） |
| 上下文管理 | Flexible Context Window (flet + LQ_t) | Session 文件 (JSON) + 消息数截断 |
