...
mod tests {
    ...
    #[tokio::test]
    async fn test_agent_logs_write_read_list() {
        let _guard = CWD_LOCK.lock().await;
        let dir = tempfile::tempdir().unwrap();
        let orig_dir = std::env::current_dir().unwrap();
        std::env::set_current_dir(&dir).unwrap();

        let write_block = vec![AgentLogsBlock {
            mode: AgentLogsMode::Write,
            filename: Some("test_log.md".into()),
            content: Some("hello world".into()),
        }];
        let results = execute(write_block).await.unwrap();
        assert_eq!(results.len(), 1);
        match &results[0].outcome {
            CommandOutcome::Success { summary } => assert!(summary.contains("test_log.md")),
            CommandOutcome::Failure { error } => panic!("{}", error),
        }

        let read_block = vec![AgentLogsBlock {
            mode: AgentLogsMode::Read,
            filename: Some("test_log.md".into()),
            content: None,
        }];
        let r = execute(read_block).await.unwrap();
        match &r[0].outcome {
            CommandOutcome::Success { summary } => assert!(summary.contains("hello world")),
            CommandOutcome::Failure { error } => panic!("{}", error),
        }

        let list_block = vec![AgentLogsBlock {
            mode: AgentLogsMode::List,
            filename: None,
            content: None,
        }];
        let r = execute(list_block).await.unwrap();
        match &r[0].outcome {
            CommandOutcome::Success { summary } => assert!(summary.contains("test_log.md")),
            CommandOutcome::Failure { error } => panic!("{}", error),
        }
    ...
    #[tokio::test]
    async fn test_agent_logs_read_nonexistent() {
        let block = vec![AgentLogsBlock {
            mode: AgentLogsMode::Read,
            filename: Some("no_such_file.md".into()),
            content: None,
        }];
        let results = execute(block).await.unwrap();
        match &results[0].outcome {
            CommandOutcome::Failure { error } => assert!(error.contains("no_such_file")),
